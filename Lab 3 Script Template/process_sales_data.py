...
def get_sales_csv_path():
    ...
    if num_params < 1:
        print("Eror Missing CSV path parameter.")
        sys.exit()
    # TODO: Check whether provide parameter is valid path of file

    csv_path = sys.argv[1]
    if not os.path.isfile(csv_path):
        print("Eror  CSV path is not in exsisting file.")
        sys.exit()
...
def process_sales_data(sales_csv_path, orders_dir_path):
    ...
    for order_id, order_df in df.groupby('ORDER ID'):
        ...
        customer_name = order_df['CUSTOMERNAME'].values[0]
        customer_name = re.sub( r'\W','', customer_name)

        order_file = f"Order{order_id}_{customer_name}.xlxs"
